...
### v1.7.0 - 고해상도 캡처 및 카메라 제어 강화
- 📷 **4K/8K 스냅샷 지원**:
  - **초고해상도**: 출판/인쇄용 4K(3840px) 및 8K(7680px) 렌더링 지원
  - **투명 배경**: 캡처 시 배경이 자동으로 투명 처리되어 디자인 활용 용이
  - **안전 모드**: GPU 메모리 한계(텍스처 크기, 총 픽셀 수) 자동 감지 및 안전 해상도 조정 (LMFP 등 복잡한 구조 실패 방지)
- 🎥 **카메라 투영 모드 토글**:
  - **Perspective(원근)**: 입체감 있는 기본 뷰
  - **Orthographic(직교)**: 왜곡 없는 정투영 뷰 (정확한 구조 비교 용이)
  - **위치 동기화**: 모드 전환 시 카메라 위치와 타겟 유지
- 🖥️ **UI/UX 개선**:
  - **하단 컨트롤 바**: 카메라 리셋, 뷰 전환, 내보내기 버튼을 직관적으로 재배치
...
- 🔋 **나트륨 이온 전지(SiB) 지원**:
  ...
  - **P2-type**: 나트륨 전용 2층 육방 구조 ($P6_3/mmc$, $Na_{0.67}MnO_2$), 삼각기둥(Prismatic) Na 자리 구현
- 🧪 **LMR (Li-rich) 구조 개선**:
  - 단사정계($C2/m$) 대신 고용체 특성을 반영한 **능면체($R\bar{3}m$)** 대칭성 적용
  - TM 층에 존재하는 리튬($Li_{TM}$)을 반투명 무광 재질로 구별하여 시각화
- 🏷️ **Dynamic Legend (동적 범례)**:
  - 현재 화면에 렌더링된 원소만 자동으로 감지하여 범례 표시 (e.g., LMR에서 미사용 원소 숨김)
  - 구조 변경 시 즉각적인 업데이트 및 우선순위 정렬
